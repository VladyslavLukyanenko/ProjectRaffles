...
{
  public class WoodWoodClient : ModuleHttpClientBase, IWoodWoodClient
  {
    private readonly IPhoneCodeService _phoneCodeService;

    public WoodWoodClient(IPhoneCodeService phoneCodeService)
    {
    ...
    public async Task<WoodWoodProductTags> GetRaffleTagsAsync(string raffleurl, CancellationToken ct)
    {
      //todo: cloudflare cookies needed for getting the page, currently user just supply raffle tag
      ...
      return new WoodWoodProductTags(tags, token);
    }
    ...
    public async Task<bool> SubmitAsync(WoodWoodSubmitPayload payload, CancellationToken ct)
    {
