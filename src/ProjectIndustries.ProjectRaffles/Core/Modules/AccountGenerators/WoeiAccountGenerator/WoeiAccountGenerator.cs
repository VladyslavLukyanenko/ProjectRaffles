...
{
  public class WoeiAccountGenerator : ProfileDependentAccountGeneratorBase
  {
    private readonly ICaptchaSolveService _captchaSolver;
    private readonly IWoeiAccountGeneratorClient _client;
    ...
    public WoeiAccountGenerator(IWoeiAccountGeneratorClient client, ICaptchaSolveService captchaSolver)
    {
      _client = client;
    ...
    public override async IAsyncEnumerable<Account> GenerateAsync([EnumeratorCancellation] CancellationToken ct = default)
    {
      while (!ct.IsCancellationRequested)
      {
        ...
        if (_addressFields.ProvinceId != null)
        {
        ...
        var captcha = await _captchaSolver.SolveReCaptchaV2Async("6LcSoR4UAAAAAO3mKqp729zO-PAz1m7DK9AqnONr",
          "https://www.woei-webshop.nl/en/services/challenge/", true, ct);
